...
CPhoenix::phoenixOptionsValues(SITE_ID, array("design", "menu", "shop", "catalog", "politic"));
...
if(!empty($arrItems))
{
	...
	foreach ($arResult["GRID"]["ROWS"] as $key => $arItem)
	{
		...
		$isOffer = isset($arOffer2Product[$arItem["PRODUCT_ID"]]);

		if($isOffer)
		{
			$productIdOfOrder = $arOffer2Product[$arItem["PRODUCT_ID"]];
			...
			if($photo)
			{
				if($arItem["DELAY"] === "Y")
				{
					...
					$img = CFile::ResizeImageGet($photo, array('width'=>280, 'height'=>136), BX_RESIZE_IMAGE_EXACT, false, Array(), false, $PHOENIX_TEMPLATE_ARRAY["ITEMS"]["DESIGN"]["ITEMS"]["PICTURES_QUALITY"]["VALUE"]);
		...
		else
		{
			...
			$arResult["GRID"]["ROWS"][$key]["NAME"] = strip_tags($arProductFields[$arItem["PRODUCT_ID"]]["~NAME"]);
