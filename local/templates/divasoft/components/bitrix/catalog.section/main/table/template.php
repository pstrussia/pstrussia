    ...
    <div class="col-xl-3 col-lg-4 col-md-6 col-12 offset-lg-0 offset-md-3 right-body">
        ...
        <? if ($arItem['QUANTITY']['QUANTITY_VALUE'] > 0 && $arItem['PRICE']['PRICE'] != '-1') { ?>
            <div class="wrapper-inner-bot row no-gutters <? if ($arItem['QUANTITY']['QUANTITY_VALUE'] > '0') { ?> hidden-js.active<? } else { ?> hidden-js <? } ?> <?= ($arItem['HAVEOFFERS']) ? "hidden" : "" ?>" id="<?= $itemIds['WR_ADD2BASKET'] ?>">
                ...
                <div class="quantity-container col-8 quantity-block" data-item="<?= $arItem['ID'] ?>">
                ...
                <div class="btn-container align-items-center col-4" id="<?= $itemIds['BASKET_ACTIONS'] ?>">
                    ...
                    <a id="<?= $itemIds['ADD2BASKET'] ?>" href="javascript:void(0);" data-item="<?= $arItem["ID"] ?>" class="main-color add2basket" title="<?= CPhoenix::prepareText($PHOENIX_TEMPLATE_ARRAY["ITEMS"]["SHOP"]["ITEMS"]["CART_BTN_ADD_NAME"]["~VALUE"]) ?>"></a>
